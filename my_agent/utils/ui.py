...
def sidebar_ui():
    ...
    # 새 채팅방 생성 버튼
    if st.button("새 채팅방", use_container_width=True, disabled=st.session_state.button_disabled_state):
        try:
    ...
    if sessions:
        ...
        for session_id, session_name in sessions:
            ...
            with col1:
                ...
                if st.button(
                    display_name, key=f"session_{session_id}", use_container_width=True, disabled=st.session_state.button_disabled_state
                ):
